...
let documentsDirectory = availableDirectories[0]
    .appendingPathComponent("Clipzy")
// migrate storage from the old app name
private let legacyDirectory = availableDirectories[0].appendingPathComponent("Pastey")
...
{
    try? FileManager.default.moveItem(at: legacyDirectory, to: documentsDirectory)
}
