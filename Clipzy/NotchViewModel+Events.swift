...
extension NotchViewModel {
    func setupCancellables() {
            ...
            .sink { [weak self] _ in
                ...
                case .opened:
                    ...
                    if !notchOpenedRect.contains(mouseLocation) {
                        ...
                        // ponytail: hardcoded 76px trash zone right of headline; SwiftUI tap handles the delete
                        let trashZone = CGRect(
                            x: notchOpenedRect.maxX - 76,
                            y: headlineOpenedRect.minY,
                            width: 44,
                            height: headlineOpenedRect.height
                        )
                        if trashZone.contains(mouseLocation), contentType == .normal, !TrayDrop.shared.isEmpty {
                            // trash click — don't cycle the menu
                        } else if let nextValue = ContentType(rawValue: contentType.rawValue + 1) {
        ...
        // peek the notch when a copy is auto-captured
        NotificationCenter.default.publisher(for: .clipzyCaptured)
            .receive(on: DispatchQueue.main)
            .sink { [weak self] _ in
                guard let self, status == .closed else { return }
                notchPop()
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) { [weak self] in
                    guard let self, status == .popping else { return }
                    notchClose()
                }
            }
            .store(in: &cancellables)
        ...
        events.keyDown
            ...
            .sink { [weak self] event in
                ...
                // delete / backspace clears current selection
                if event.keyCode == 51, !TrayDrop.shared.selection.isEmpty {
                    TrayDrop.shared.deleteSelected()
                }
            }
            .store(in: &cancellables)
